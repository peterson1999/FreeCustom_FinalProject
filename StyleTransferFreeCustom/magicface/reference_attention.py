...
class ReferenceAttention():
    def __init__(self,  start_step=0, end_step=50, step_idx=None, layer_idx=None, ref_masks=None, mask_weights=[1.0,1.0,1.0], inject_step=20, viz_cfg=None, ref_index=None,feature_mapping = None):
        self.cur_step       =  0
        ...
        self.num_refs = len(ref_masks) if ref_masks is not None else 0
        self.style_ref_idx = self.num_refs - 1  # Last reference is style reference
        self.style_target_idx = self.num_refs - 2 

        self.ref_masks    = ref_masks
        self.mask_weights = mask_weights
        target_token = ref_index[self.style_target_idx]  # Get token position for style target (e.g., hat's token position)
        self.feature_mapping = {
            target_token: self.style_ref_idx  # Map target token to use style reference features
        }
    ...
    def mrba_forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, controller, **kwargs):
        ...
        if controller.mask is not None and self.cur_step > self.inject_step:
            ...
            mask_t = torch.index_select(mask_t, 0, torch.tensor(self.ref_index, dtype=torch.long, device='cuda:6'))
            ...
            for i in range(len(mask_t)):
                ...
                token_pos = self.ref_index[i]
                if token_pos in self.feature_mapping:  # Hat token
                    # Use hat's self-attention for structure
                    hat_sa = self.sa_forward(q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs)
                    hat_sa_u, hat_sa_c = hat_sa.chunk(2)
                    
                    # Use style reference features for texture
                    k_style = kc_r[num_heads * self.style_ref_idx: num_heads * (self.style_ref_idx + 1),:,:]
                    v_style = vc_r[num_heads * self.style_ref_idx: num_heads * (self.style_ref_idx + 1),:,:]
                    
                    # Generate texture features for all positions
                    texture_out = self.attn_batch_rba(qc_o, 
                        torch.cat([kc_o, k_style], 1),
                        torch.cat([vc_o, v_style], 1),
                        None, None, is_cross, place_in_unet, num_heads,
                        attn_batch_type='mrsa', index=i, **kwargs)
                    
                    # Create a zero tensor with same shape as hat_sa_c
                    blended = torch.zeros_like(hat_sa_c)
                    # Only blend in regions where mask_i is True
                    mask_bool = mask_i == 1
                    # Blend structure and texture
                    blended[:, mask_bool, :] = 0.7 * hat_sa_c[:, mask_bool, :] + 0.3 * texture_out[:, mask_bool, :]
                    # Keep original structure where mask is False
                    blended[:, ~mask_bool, :] = hat_sa_c[:, ~mask_bool, :]
                    
                    out_c[0] = blended[0]  # Use first head's output
                else:
                    # Regular feature injection for other regions
                    k_features = kc_r[num_heads * i: num_heads * (i + 1),:,:]
                    v_features = vc_r[num_heads * i: num_heads * (i + 1),:,:]
                    out = self.attn_batch_rba(qc_o,
                        torch.cat([kc_o, k_features], 1),
                        torch.cat([vc_o, v_features], 1),
                        None, None, is_cross, place_in_unet, num_heads,
                        attn_batch_type='mrsa', index=i, **kwargs)
                    # Apply mask
                    out_c[0, mask_i==1, :] = out[0, mask_i==1, :]
