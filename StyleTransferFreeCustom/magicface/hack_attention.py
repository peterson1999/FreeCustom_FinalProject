...
class MyCrossAttnProcessor:
    ...
    def __call__(self, attn: Attention, x, encoder_hidden_states=None, attention_mask=None, context=None, mask=None, **kwargs):
        ...
        self.attnstore(torch.index_select(rearrange(sim_soft, '(b h) s t -> b h s t', h=attn.heads).mean(1), 0, torch.tensor([0, x.shape[0] // 2], dtype=torch.long, device='cuda:6')), is_cross, self.place_in_unet)
        # the only difference
        out = self.mrsa(
