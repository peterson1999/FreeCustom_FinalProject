...
from pipelines.pipeline_stable_diffusion_magicface import StableDiffusionMagicFacePipeline
from magicface.reference_attention import ReferenceAttention
from magicface.hack_attention import register_attention_control, AttentionStore
import shutil
...
if __name__ == "__main__":
    ...
    model = StableDiffusionMagicFacePipeline.from_pretrained(cfg.model_path, scheduler=scheduler, local_files_only=False).to(device)
    ...
    for seed in cfg.seeds:
        ...
        mrsa = ReferenceAttention(
                                start_step     = cfg.start_step,
                                end_step       = cfg.end_step,
                                layer_idx      = cfg.layer_idx, 
                                ref_masks      = ref_masks,
                                mask_weights   = cfg.mask_weights,
                                inject_step = cfg.inject_step,
                                viz_cfg        = viz_cfg,
                                ref_index = cfg.ref_index)
        controller = AttentionStore(H = model.unet.config.sample_size)
        register_attention_control(model, controller, mrsa)
