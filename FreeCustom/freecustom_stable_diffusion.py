...
def load_model(cfg, scheduler, device):
...
if __name__ == "__main__":
    ...
    cfg = OmegaConf.load(f"configs/config_stable_diffusion.yaml")
    print(f'config: {cfg}')
    ...
    LOCAL_MODEL_PATH = "models/stable_diffusion_model"  # adjust path as needed

    # Create directory if it doesn't exist
    os.makedirs(LOCAL_MODEL_PATH, exist_ok=True)
    #model = StableDiffusionFreeCustomPipeline.from_pretrained(cfg.model_path, scheduler=scheduler,
    #                                                          mirror="https://hf-mirror.com").to(device)
    model = load_model(cfg, scheduler, device)
